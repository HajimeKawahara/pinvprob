def tikhonov_regularization(Dm, Ao, Smprior, U, VT, S, lambdatik, p=None):
    ...
    for i in range(0, p):
        ...
        dxidlam += (1.0 - phii) * (phij**2) * (np.inner(U[:, i], dv)**2)
    ...
    curv_lcurve = -2.0 * xi * rho / dxidlam * (
        lam2 * dxidlam * rho + 2.0 * lambdatik * xi * rho +
        lam2 * lam2 * xi * dxidlam) / (
            (lam2 * lam2 * xi * xi + rho * rho)**1.5)
